...
class NexusTester:
    ...
    def register(self):
        ...
        try:
            ...
            if r.status_code == 201:
                ...
                self.log("AUTH", f"Falha no registro: {r.status_code} - {r.text}", "ERROR")
                return False
    ...
    def login(self):
        ...
        self.log("AUTH", f"Tentando realizar login com email: {self.user_data['email']}")
        ...
        if r.status_code == 200:
            ...
            if self.token:
                self.session.headers.update({'Authorization': f'Bearer {self.token}'}) # Adjust prefix if needed
                self.log("AUTH", f"Login sucesso. Token obtido: {self.token[:10]}...", "SUCCESS")
            return True
        else:
    ...
    def list_files(self):
        ...
        if r.status_code == 200:
            ...
            self.log("VAULT", f"Listagem sucesso. Total de arquivos: {len(items)}", "SUCCESS")
            
            # Valida se o arquivo criado está na lista
            found = any(f['id'] == self.file_id for f in items)
            if found:
                self.log("VAULT", "Arquivo recém-criado foi encontrado na lista.", "SUCCESS")
            else:
                self.log("VAULT", "ALERTA: Arquivo criado não encontrado na lista!", "WARN")
            return True
        else:
