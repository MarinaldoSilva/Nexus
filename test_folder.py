...
reg = requests.post("http://localhost:8000/api/auth/register/", json={
    "username": username,
...
r1 = requests.post("http://localhost:8000/api/files/folders/", headers=headers, json={"name": "Documentos"})
...
r2 = requests.post("http://localhost:8000/api/files/folders/", headers=headers, json={
    "name": "Trabalho",
    "parent": folder1['id']
})
...
if r2.status_code == 201:
    folder2 = r2.json()
    print(f"✅ Subpasta criada: {folder2.get('name')} (parent: {folder2.get('parent')})")
else:
