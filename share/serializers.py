...
class SharedLinkSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        extra_kwargs = {"expired": {"required": False, "allow_null": True}}
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if not expired:
            ...
            raise serializers.ValidationError(
                "A data de expiração deve ser posterior a atual."
            )

        if bool(file) == bool(folder):
            raise serializers.ValidationError(
                "É necessário ao menos m ficheiro para ser gerado o link."
            )
        return attrs
